/* UI controller for the isolated background engine. */
(() => {
  ...
  function render(data){
    ...
    document.getElementById('results').hidden=false;progress(100,'Background comparison completed. Review verification gates before use.');
    const renderDuration=performance.now()-renderStart;bufferedLog(`DOM render completed in ${renderDuration.toFixed(1)} ms`);bufferedLog(`Worker total: ${Number(data.performance?.totalDuration||0).toLocaleString()} ms`);bufferedLog(`Root cause fixed: ${data.performance?.rootCause||'bounded worksheet parsing'}`);
    window.GTMDJD_PERFORMANCE={worker:data.performance,renderDuration,longTasks:[...longTasks],completedAt:new Date().toISOString()};
    document.getElementById('dlMaster').onclick=()=>save(new Blob([csvText(S.master)],{type:'text/csv;charset=utf-8'}),'GTMDJD_FINAL_MASTER.csv');
    document.getElementById('dlExceptions').onclick=()=>save(new Blob([csvText(S.exceptions)],{type:'text/csv;charset=utf-8'}),'GTMDJD_EXCEPTION_QUEUE.csv');
    document.getElementById('dlWorkbook').onclick=()=>{const start=performance.now(),w=XLSX.utils.book_new();XLSX.utils.book_append_sheet(w,XLSX.utils.json_to_sheet(S.master),'Final Master');XLSX.utils.book_append_sheet(w,XLSX.utils.json_to_sheet(S.exceptions),'Exception Queue');XLSX.utils.book_append_sheet(w,XLSX.utils.json_to_sheet(S.coverage),'Coverage');XLSX.writeFile(w,'GTMDJD_BOSS_READY.xlsx');bufferedLog(`Workbook export completed in ${(performance.now()-start).toFixed(1)} ms`);};
    document.getElementById('dlZip').onclick=async()=>{const z=new JSZip();z.file('GTMDJD_FINAL_MASTER.csv',csvText(S.master));z.file('GTMDJD_EXCEPTION_QUEUE.csv',csvText(S.exceptions));z.file('GTMDJD_PERFORMANCE.json',JSON.stringify(window.GTMDJD_PERFORMANCE,null,2));save(await z.generateAsync({type:'blob'}),'GTMDJD_RESULTS.zip');};
  }
  ...
  runButton.onclick=async()=>{
    ...
    runButton.disabled=true;runButton.textContent='Running in background…';document.getElementById('results').hidden=true;document.getElementById('log').textContent='';longTasks.length=0;progress(2,'Starting isolated background run...');
    ...
    try{
      ...
      worker=new Worker(`worker-engine-schemafix.js?v=20260727-schemafix1`);
