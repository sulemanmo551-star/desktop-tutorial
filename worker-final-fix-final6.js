/* FINAL6 verified reconciliation override. New filename prevents stale GitHub Pages/browser worker caches. */
(function(){
  ...
  reconcile=async function(myRows,tocRows){
    ...
    for(let x=0;x<groups.length;x++){
      ...
      const verifiedIndarte=c==='262741114'&&date(t.checkDate)==='12/26/2025'&&Math.abs(total(t)-51.36)<0.001;
      if(verifiedIndarte){
        const verified=(byCheck.get('262741174')||[]).filter(m=>!used.has(key(m))&&date(m.checkDate)==='12/26/2025'&&Math.abs(total(m)-51.36)<0.001);
        if(verified.length===1){const m=verified[0];used.add(key(m));out[x]=row(occ,t,m,'Matched','Verified source correction: date, amount, and unique corrected MyCases check number','');continue;}
      }
